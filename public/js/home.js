...
export async function show() {
  ...
  render(h('div',
    ...
    h('p.lede', personalRole(role)
      ? 'Your record lives here, and you help write it.'
      : 'Everything below is computed on this machine from the records in your own folder.'),
    personal,
    personalRole(role) ? h('p.notice', { style: 'margin-top:18px' },
      'Everything below this line is district-wide totals for every school, not your own record. No names are shown, and nobody else\u2019s record is visible here.') : null,
    h('div.grid.four', { style: 'margin-top:14px' },
      stat(stats.students, personalRole(role) ? 'Students district-wide' : 'Students'),
      stat(stats.fragments, personalRole(role) ? 'Fragments district-wide' : 'Fragments'),
      stat(stats.schools?.length || 0, 'Schools'),
      stat(stats.staff, 'Staff'),
    ),
...
async function personalCard(role) {
  if (role === 'student') {
    return h('div.card', { style: 'margin-top:14px' },
      h('h2', { style: 'margin-top:0' }, 'Your page'),
      h('p.small.muted', 'Everything your school records about you, plus whatever you want to add yourself: how a week went, work you are proud of, a photo.'),
      h('button.btn', { onclick: () => go('/me') }, 'Open my record'));
  }
  if (role === 'family') {
    const { people } = await api('/api/people?limit=50');
    return h('div.card', { style: 'margin-top:14px' },
      h('h2', { style: 'margin-top:0' }, 'My kids'),
      people.length ? h('div.row', people.map((p) => h('button.btn.ghost', { onclick: () => go('/person/' + p.id) },
        `${displayName(p)} · grade ${gradeLabel(p.grade)}`))) : h('p.muted.small', 'No records are linked to this account yet. Ask the office to link them.'));
  }
  return null;
}
...
function outcomeCard(stats, role) {
  ...
  return h('div.card',
    h('h2', { style: 'margin-top:0' }, personalRole(role) ? 'How students across the district are doing' : 'How students are doing'),
    h('p.small.muted', { style: 'margin-top:0' }, 'Patterns, not verdicts. Each one is a question for a person who knows the child.'),
...
function radarCard(stats, dims) {
  if (!dims.length) return h('div.card', h('h2', 'Signals'), h('p.muted.small', 'The schema did not load.'));
  const present = dims.filter((d) => stats.dimMeans?.[d.key] != null).length;
  return h('div.card',
    h('h2', { style: 'margin-top:0' }, 'District signal shape'),
    h('p.small.muted', { style: 'margin-top:0' }, `Mean of each dimension across ${stats.students} students. ${present} of ${dims.length} dimensions have data.`),
...
function schoolsCard(stats, role) {
  ...
  return h('div.card', { style: 'margin-top:18px' },
    h('h2', { style: 'margin-top:0' }, 'Schools'),
    bars(schools.map((s) => ({ label: s.name || s.id, value: s.students / max, note: `${s.students} students` }))),
